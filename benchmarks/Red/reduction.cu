...
reduceBlock(volatile DATATYPE *sdata, DATATYPE mySum, const unsigned int tid, bool recover)
{
    ...
    if (blockSize >= 512 && tid < 256)
    {
        if (tid >= 128)
        {
            ...
            asm volatile("st.relaxed.cta.u8 [%0], 1;":: "l"(&lock[tid]));
    ...
    if (blockSize >= 256 && tid < 128)
    {
        ...
        while(blockSize >= 512 && val != 1) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 128]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
    ...
    if (blockSize >= 128 && tid < 64)
    {
        ...
        while(blockSize >= 256 && val != 1) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 64]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
    ...
    if (tid < 32)
    {
        ...
        while(blockSize >= 128 && val != 1) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 32]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
        ...
        while(blockSize >= 64 && val != 1 && tid < 16) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 16]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
        ...
        while(blockSize >= 32 && val != 1 && tid < 8) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 8]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
        ...
        while(blockSize >= 16 && val != 1 && tid < 4) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 4]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
        ...
        while(blockSize >= 8 && val != 1 && tid < 2) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 2]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
        ...
        while(blockSize >= 4 && val != 1 && tid == 0) {
    		asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(&lock[tid + 1]));
#ifdef GPM_EPOCH_MODEL
	    __threadfence();
#endif 
	}
...
reduceBlocks(const DATATYPE *g_idata, volatile DATATYPE *g_odata, 
	volatile DATATYPE *sdata, unsigned int n, bool recover)
{
...
template <unsigned int blockSize, bool nIsPow2>
__global__ void reduceSinglePass(const DATATYPE *g_idata, volatile DATATYPE *g_odata, 
	volatile DATATYPE *sdata, unsigned int n, bool recover)
{
...
void reduceSinglePasses(int size, DATATYPE *d_idata, DATATYPE *d_odata, 
	int nThreads, int nBlocks, DATATYPE *smem, bool recover = false)
{
...
// Kernel to simulate just the "UNDO" portion of recovery. 
// Full recovery (UNDO + REDO) is encapsulated in main kernel
__global__ void recoveryKernel(const DATATYPE *g_idata, volatile DATATYPE *g_odata, 
	volatile DATATYPE *sdata, unsigned int n) {
	__shared__ volatile char lock[NTHREADS];
...
int main(int argc, char **argv)
{
	...
	recoveryKernel<<<nBlocks, nThreads>>>(d_idata, d_odata, d_smem, step_size);
