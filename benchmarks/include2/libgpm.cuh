 ...
 */
static __device__ void srp_order(void)
{
 ...
 * gpm_drain -- wait for any PM stores to drain from HW buffers
 */
static __device__ void srp_persist(void)
{
...
static __device__ __inline__ void release_block(char *loc, char val) {
	asm volatile("st.relaxed.cta.u8 [%0], %1;":: "l"(loc), "r"((int)val));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
}

static __device__ __inline__ int acquire_block(char *loc) {
	int val;
	asm volatile("ld.relaxed.cta.u8 %0, [%1];" : "=r"(val) : "l"(loc));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
	return val;
...
static __device__ __inline__ void release_block(int *loc, int val) {
	asm volatile("st.relaxed.cta.s32 [%0], %1;":: "l"(loc), "r"(val));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
}

static __device__ __inline__ int acquire_block(int *loc) {
	int val;
	asm volatile("ld.relaxed.cta.s32 %0, [%1];" : "=r"(val) : "l"(loc));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
	return val;
...
static __device__ __inline__ void release_device(char *loc, char val) {
	asm volatile("st.relaxed.gpu.u8 [%0], %1;":: "l"(loc), "r"((int)val));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
}

static __device__ __inline__ int acquire_device(char *loc) {
	int val;
	asm volatile("ld.relaxed.gpu.u8 %0, [%1];" : "=r"(val) : "l"(loc));
#ifdef GPM_EPOCH_MODEL
    __threadfence(); 
#endif 
	return val;
