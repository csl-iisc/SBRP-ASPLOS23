    ...
    if(!recovery) {
       ...
       //#endif 
        ...
        for (iter=0; iter< niter; iter++){
			...
			setMemory<<<30, 1024>>>(C_cuda, size_I, -1.0);
			setMemory<<<30, 1024>>>(J_cuda_out, size_I, -1.0);
			...
			int kernel_start = INT_MAX;
			int *d_kernel_start;
			cudaMalloc((void **)&d_kernel_start, sizeof(int));
			cudaMemcpy(d_kernel_start, &kernel_start, sizeof(int), cudaMemcpyHostToDevice);
	    	recovery_kernel<<<dimGrid, dimBlock>>>(C_cuda, J_cuda, d_kernel_start, cols, rows);
			cudaMemcpy(&kernel_start, d_kernel_start, sizeof(int), cudaMemcpyDeviceToHost);
