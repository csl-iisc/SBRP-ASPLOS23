...
inline __device__ uint scan1Inclusive(uint idata, volatile uint *s_Data, uint size, char *locks)
{
    ...
    for (ctr = 0; (1 << ctr) < size; ctr++)
    {
		...
		//if(tid + offset / 2 < THREADBLOCK_SIZE || tid >= offset) {
		    ...
		    if(tid >= offset) {
		    	while(acquire_device(&locks[tid + size * copy - offset]) != ctr);
				t += s_Data[tid + beg * copy - offset];
				release_device(&locks[tid + size * copy - offset], (1 << 7) | ctr);
			}
	    //}
	    
		//__syncthreads();
		if(tid + offset < THREADBLOCK_SIZE) {
			while(!((acquire_device(&locks[tid + size * copy])) & (1 << 7)));
		}
		//if(tid + offset / 2 < THREADBLOCK_SIZE || tid >= offset) {
			// Write to copy array
			s_Data[tid + beg * ((copy + 1) % COPIES)] = t;
			// Have thread release
			release_device(&locks[tid + size * ((copy + 1) % COPIES)], ctr + 1);
		//}
    }
	//__syncthreads();
